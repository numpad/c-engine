	...
	if (is_command) {
		...
		if (strncmp(input, "/sessions", strlen("/sessions")) == 0) {
			...
			for (int i = 0; i < sessions_len; ++i) {
				...
				messagequeue_add("Session #%d (%d): In Lobby: %d (%p)", i, s->id, s->is_in_lobby_with_id, s);
			}
...
static void create_lobby(struct lobby_create_request *msg, struct session *requested_by) {
	...
	response.create_error = 0;
	messagequeue_add("Moving from Lobby %d to %d", requested_by->is_in_lobby_with_id, msg->lobby_id);
	requested_by->is_in_lobby_with_id = msg->lobby_id;
	messagequeue_add("#%06d created lobby %d!", requested_by->id, requested_by->is_in_lobby_with_id);
...
static void list_lobbies(struct lobby_list_request *msg, struct session *requested_by) {
	...
	for (int i = 0; i < sessions_len; ++i) {
		...
		if (lobby_id >= 0) {
			...
			res.ids_of_lobbies[i] = lobby_id;
			messagequeue_add("have lobby %d", lobby_id);
		}
	}
	cJSON *json = cJSON_CreateObject();
